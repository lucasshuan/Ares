...
export const accounts = pgTable(
  ...
  (table) => ({
    accountsProviderAccountIdx: uniqueIndex("accounts_provider_account_idx").on(table.provider, table.providerAccountId),
...
export const verificationTokens = pgTable(
  ...
  (table) => ({
    verificationTokensIdentifierTokenIdx: uniqueIndex("verification_tokens_identifier_token_idx").on(table.identifier, table.token),
  }),
);
...
export const players = pgTable(
  ...
  (table) => ({
    ...
    playersGameUserIdx: uniqueIndex("players_game_user_idx").on(table.gameId, table.userId),
    playersPrimaryUsernameIdIdx: index("players_primary_username_id_idx").on(table.primaryUsernameId),
  }),
);
...
export const playerUsernames = pgTable(
  ...
  (table) => ({
    playerUsernamesPlayerUsernameIdx: uniqueIndex("player_usernames_player_username_idx").on(table.playerId, table.username),
    playerUsernamesPlayerIdIdx: index("player_usernames_player_id_idx").on(table.playerId),
  }),
);
...
export const rankings = pgTable(
  "rankings",
  {
    ...
    inactivityThresholdHours: integer("inactivity_threshold_hours").default(120).notNull(),
    inactivityDecayFloor: integer("inactivity_decay_floor").default(1000).notNull(),
  ...
  (table) => ({
    rankingsGameNameIdx: uniqueIndex("rankings_game_name_idx").on(table.gameId, table.name),
    rankingsGameSlugIdx: uniqueIndex("rankings_game_slug_idx").on(table.gameId, table.slug),
    rankingsGameIdIdx: index("rankings_game_id_idx").on(table.gameId),
    rankingsIsApprovedIdx: index("rankings_is_approved_idx").on(table.isApproved),
...
export const rankingEntries = pgTable(
  ...
  (table) => ({
    rankingEntriesRankingPlayerIdx: uniqueIndex("ranking_entries_ranking_player_idx").on(table.rankingId, table.playerId),
    rankingEntriesRankingIdIdx: index("ranking_entries_ranking_id_idx").on(table.rankingId),
    rankingEntriesPlayerIdIdx: index("ranking_entries_player_id_idx").on(table.playerId),
  }),
);
...
export const resultEntries = pgTable(
  ...
  (table) => ({
    resultEntriesResultPlayerIdx: uniqueIndex("result_entries_result_player_idx").on(table.resultId, table.playerId),
    resultEntriesResultIdIdx: index("result_entries_result_id_idx").on(table.resultId),
    resultEntriesPlayerIdIdx: index("result_entries_player_id_idx").on(table.playerId),
  }),
);
...
export const resultAttachments = pgTable(
  ...
  (table) => ({
    resultAttachmentsResultIdIdx: index("result_attachments_result_id_idx").on(table.resultId),
...
export const userPermissions = pgTable(
  ...
  (table) => ({
    ...
    userPermissionsUserIdIdx: index("user_permissions_user_id_idx").on(table.userId),
    userPermissionsPermissionIdIdx: index("user_permissions_permission_id_idx").on(table.permissionId),
  }),
);
...
export const playerUsernamesRelations = relations(playerUsernames, ({ one }) => ({
  player: one(players, { fields: [playerUsernames.playerId], references: [players.id] }),
}));
...
export const rankingEntriesRelations = relations(rankingEntries, ({ one }) => ({
  ranking: one(rankings, { fields: [rankingEntries.rankingId], references: [rankings.id] }),
  player: one(players, { fields: [rankingEntries.playerId], references: [players.id] }),
}));

export const resultsRelations = relations(results, ({ one, many }) => ({
  ranking: one(rankings, { fields: [results.rankingId], references: [rankings.id] }),
...
export const resultEntriesRelations = relations(resultEntries, ({ one }) => ({
  result: one(results, { fields: [resultEntries.resultId], references: [results.id] }),
  player: one(players, { fields: [resultEntries.playerId], references: [players.id] }),
}));

export const resultAttachmentsRelations = relations(resultAttachments, ({ one }) => ({
  result: one(results, { fields: [resultAttachments.resultId], references: [results.id] }),
}));
